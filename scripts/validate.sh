...
find ./clusters -maxdepth 2 -type f -name '*.yaml' -print0 | while IFS= read -r -d $'\0' file;
  do
    kubeval -v 1.23.3 ${file} --strict --ignore-missing-schemas --additional-schema-locations=file:///tmp/flux-crd-schemas
...
find . -type f -name $kustomize_config -print0 | while IFS= read -r -d $'\0' file;
  do
    ...
    kustomize build "${file/%$kustomize_config}" $kustomize_flags | \
      kubeval -v 1.23.3 --ignore-missing-schemas --strict --additional-schema-locations=file:///tmp/flux-crd-schemas
    if [[ ${PIPESTATUS[0]} != 0 ]]; then
      exit 1
    fi
done
